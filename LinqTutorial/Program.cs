...
namespace LinqTutorial
{
    class Program
    {
        ...
        static void X()
        {
            ...
            Console.WriteLine($"Biggest number is {ints.Max()}");
            Console.WriteLine($"Smallest number is {ints.Min()}");
            var sum = ints.Sum() - ints.Max() - ints.Min();
            var avg = sum / (ints.Count() - 2.0);
            Console.WriteLine($"Average without max and min is {avg}");
            //var lessthanavg = ints.Where(i => i <= ints.Average()).OrderByDescending(j => j);
            //foreach (var i in lessthanavg)
            //{
            //    Console.Write($"{i} ");
            //}
            //non lambda way, the query way
            ...
            var lowInts = (from i in ints
                           ...
                           select i).Sum();
            Console.WriteLine($"Sum is {lowInts}");
        }
    }
}
